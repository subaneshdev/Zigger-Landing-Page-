    ...
    try {
      ...
      if (targetCode) {
        const { data: members } = await supabaseAdmin
          .from('profiles')
          .select('id, full_name, mobile, created_at, referral_earnings')
        ...
        if (members && members.length > 0) {
          referredMembers = members.map((m) => ({
            ...
            status: m.referral_earnings > 0 ? 'REWARD_CLAIMED' : 'PENDING_FIRST_ZIG',
            reward_earned: m.referral_earnings > 0 ? `₹${m.referral_earnings}` : '₹20 (Pending Gig)'
          }));

          // Try updating counts in organizations table if columns exist
          if (organizationData && organizationData.id) {
            const activeCount = referredMembers.filter(m => m.status === 'REWARD_CLAIMED').length;
            await supabaseAdmin
              .from('organizations')
              .update({
